...
public class Configuration {
  ...
  // 类型处理器注册表，内置许多，可以通过<typeHandlers>节点补充：类型处理器注册
  // JDBC类型与对应类型处理器的映射: jdbcTypeHandlerMap
  // Java类型与Map<JdbcType, TypeHandler<?>>的映射: typeHandlerMap
  // 所有的类型处理器(allTypeHandlersMap): 键为typeHandler.getClass() ，值为typeHandler
